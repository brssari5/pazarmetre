...
async def dashboard(request: Request):
    ...
    with get_session() as s:
        ...
        for p in prods:
            ...
            # SON 7 GÜNDEN ESKİLERİ AT + marka bazında en yeni teklifi tut
...
async def product_detail(request: Request, name: str):
    ...
    rows = dedupe_by_brand_latest(rows)
    if not rows:
        return layout(request, "<div class='bg-white card p-6'>Bu ürün için geçerli fiyat bulunamadı.</div>", prod.name)
