...
<body class="bg-gradient-to-b from-brand-50 to-white text-gray-900">
  <div class="max-w-6xl mx-auto p-4">
    ...
    <footer class="mt-10 text-xs text-gray-400 text-center">
      <a href="/iletisim" class="text-indigo-600 hover:underline mr-4">İletişim</a>
      <a href="/hukuk" class="text-indigo-600 hover:underline mr-4">Hukuki Bilgilendirme</a>
      <span class="text-gray-400 block mt-2">© {datetime.utcnow().year} Pazarmetre · Fiyatlar bilgilendirme amaçlıdır.</span>
    </footer>
  </div>
</body></html>"""
...
async def admin_step1(request: Request):
    ...
    # Fiyat kaynağı değişmiş olanlar (price_watcher işaretleyecek)
    with get_session() as s:
        try:
            # True/1 ayrımı olmasın diye her ikisini de kabul et
            bad_count = s.exec(
                select(func.count())
                .select_from(Offer)
            ...
            # Kolon daha eklenmemişse ya da başka bir hata varsa admin sayfası yine açılsın
...
<div id="gizlilik" class="tab-content">
  ...
  <p class="text-gray-700 mb-3">
    Pazarmetre, kullanıcılarından herhangi bir kişisel veri talep etmez.
    Sitemizdeki fiyatlar kamuya açık market sitelerinden veya yerel işletmelerin bildirdiği fiyatlardan alınır.
  </p>
  <p class="text-gray-700 mb-3">
    Ziyaretiniz sırasında sistem güvenliği amacıyla IP adresi, tarayıcı bilgisi gibi teknik veriler kaydedilebilir.
    Bu veriler kimlik belirlemeye yönelik değildir ve üçüncü kişilerle paylaşılmaz.
  </p>
    ...
    <a href="mailto:pazarmetre1@gmail.com" class="text-indigo-600 underline">pazarmetre1@gmail.com</a>
      ...
      <div id="sorumluluk" class="tab-content hidden">
        ...
        <p class="text-gray-700 mb-3">
          ...
          Bu fiyatlar sözlü (telefon) veya yazılı (e-posta, WhatsApp) olarak alınabilir.
        </p>
        <p class="text-gray-700 mb-3">
