   ...
   # URL’den kategori filtresi
    ...
    with get_session() as s:
        ...
        for p in prods:
            ...
            # Kategorisi tanımsız veya bilmediğimiz bir şeyse atla
            if cat_key not in cards_by_cat:
                continue

            # Eğer kullanıcı belirli kategori seçtiyse ona göre filtre
    ...
    if selected_cat == "et":
        ...
        sections_html = make_section("Diğer ürünler", "🛒", cards_by_cat["diger"])
    else:
        sec_et = make_section("Kırmızı Et", "🥩", cards_by_cat["et"])
        sec_tavuk = make_section("Tavuk", "🍗", cards_by_cat["tavuk"])
        sec_diger = make_section("Diğer ürünler", "🛒", cards_by_cat["diger"])
...
async def admin_step1(request: Request):
    ...
    with get_session() as s:
        ...
        try:
            ...
            total_visits = total_row[0] if isinstance(total_row, tuple) else total_row

            # Son 24 saat için eşik
            cutoff = datetime.utcnow() - timedelta(days=1)

            last24_row = s.exec(
                select(func.count())
                .where(Visit.ts >= cutoff)
            ).one()
            last24_visits = last24_row[0] if isinstance(last24_row, tuple) else last24_row
