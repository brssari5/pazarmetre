...
class Product(SQLModel, table=True):
    ...
    category: Optional[str] = None  # "et" veya "tavuk"
...
async def dashboard(request: Request):
    ...
    # URL'den kategori filtresi (hepsi / et / tavuk)
    selected_cat = request.query_params.get("cat", "hepsi").lower()
    if selected_cat not in ("hepsi", "et", "tavuk"):
        selected_cat = "hepsi"

    # Sekme butonları
    tabs = []
    for slug, label in [("hepsi", "Hepsi"), ("et", "Kırmızı Et"), ("tavuk", "Tavuk")]:
        active = "bg-emerald-600 text-white border-emerald-600" if selected_cat == slug else "bg-white text-gray-700 border-gray-200"
    ...
    with get_session() as s:
        ...
        for p in prods:
            # Sadece et / tavuk kategorili ürünler (diğerlerini şimdilik atlıyoruz)
            cat_key = (p.category or "").lower()
            if cat_key not in ("et", "tavuk"):
                continue

            # Bu ürün için, bu lokasyondaki tüm teklifler
            q_off = (
                select(Offer, Store)
            ...
            if nb:
                rows_nb = [
                    (o, st) for (o, st) in rows
                    if (st.neighborhood or "").lower() == nb.lower()
                ]
                if rows_nb:
                    rows = rows_nb

            # 👇 ÖNCEKİ: SON 7 GÜNDEN ESKİLERİ ATıyorduk
            # rows = only_fresh_and_latest(rows, days_stale=DAYS_HARD_DROP)
            # ŞİMDİ: Eski fiyatları SİLMIYORUZ, sadece marka bazında en yeni teklifi tutuyoruz
            rows = dedupe_by_brand_latest(rows)

            # Filtrelerden sonra hiç geçerli fiyat kalmadıysa kart oluşturma
            if not rows:
                continue
            ...
            # Kaç günlük?
            days_old = (datetime.utcnow().date() - off.created_at.date()).days
            is_old = days_old >= DAYS_HARD_DROP  # 7 gün ve üzeri için uyarı
            ...
            # 7+ günlük fiyat uyarısı
            old_badge = ""
            if is_old:
                old_badge = (
                    "<div class='text-[11px] text-amber-600 mt-1'>"
                    "⚠ Bu fiyat 7 günden eski, güncel olmayabilir."
                    "</div>"
                )
    ...
    if selected_cat == "et":
        ...
        sections_html = sec_et + sec_tavuk
...
async def admin_step1(request: Request):
    ...
    <div class="bg-white card p-6 max-w-xl mx-auto">
      ...
      <h2 class="text-lg font-bold mb-3">1) Mağaza Adını Gir</h2>
...
async def admin_bulk_save(
    ...
    # (ürün adı, fiyat, adres, url, gram, unit, kategori)
    entries: List[Tuple[str, float, str, str, float | None, str | None, str | None]] = []

    for pn, pr, addr, src, sw_raw, su_raw, cat in zip_longest(
        product_name,
        price,
        store_address,
        source_url,
        ...
        entries.append((pn, pv, addr, src, sw, su, cat))
    ...
    with get_session() as s:
        ...
        for target_dist in target_districts:
            ...
            for pn, pv, addr, src, sw, su, cat in entries:
                p = s.exec(select(Product).where(Product.name == pn)).first()
                if not p:
                    # yeni ürün: kategori ile birlikte oluştur
                    p = Product(
                        name=pn,
                        unit="kg",
                    ...
                    # mevcut ürün: gerekiyorsa featured / category güncelle
                    updated = False
